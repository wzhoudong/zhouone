...
$(function() {
    //index.html/bag.html列表页 加载处
function images_down(main_c, img_d,url,oresult) {
        var omain_c = $(main_c);
        var oimg_d = $(img_d);
        var counter = 0;
        // 每页展示4个
        ...
        omain_c.dropload({
            ...
            loadDownFn: function(me) {
				...
				if(oresult=='1'){       
                        ...
                        if(oresult=='2' || oresult=='3'){              
                              result += '<li class="'+data.list[i].cid+'"><img src="' + data.list[i].path + '"  >' + '<p class="bag_head_tit">' + data.list[i].title + '</p></li>'
                        }								
                        ...
                        setTimeout(function() {
                            ...
                            me.resetload();
						if(oresult=='2'){    
						 	     bag_list(".bag_list_up li","bag")}
						if(oresult=='3'){    
						 	     bag_list(".bag_list_up2 li","bag2")}
                        }, 10);
                    },
     ...
     if ($("ul").hasClass('img_down')) {
        images_down('.web_main', '.img_down',''+getIndexListUrl+'','1') //index.html
    }
     if ($("ul").hasClass('bag_list_up')) {
        images_down('.bag_list', '.bag_list_up',''+getBagListUrl+'','2') //bag.html
     }
     if ($("ul").hasClass('bag_list_up2')) {
        images_down('.bag_list2', '.bag_list_up2',''+getBagsListUrl+'','3') //bag2.html
     }
  //加载bag.html banner 数据

    function bag_list(bag_list_up,name) {
 		var obag_list_up=$(bag_list_up);
            obag_list_up.each(function() {
            $(this).on('click', function() {
                  var cid = $(this).attr('class');
                  var index=$(this).index();
                  if(name=="bag"){
			bag_banner(index,""+getBagListClickUrl+""+cid+"")	
			}
			if(name=="bag2"){
			bag_banner(index,""+getBagsListClickUrl+""+cid+"")	
			}
 ...
 //bag.html  banner
function bag_banner(that,banner_url){
$.ajax({
type:"GET", 
url:banner_url, 
dataType:"json", 
...
var music="<ul>"; 
    for (j = 0; j < data.list.length; j++) {
                  music += "<li> <img src='" + data.list[j].path + "'></li>";
      }
music+="</ul>";  
